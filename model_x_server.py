...
while True:
    ...
    for box in all_boxes:
        if not box.has_fb:
            curr_cls = box.cls_id
            curr_conf = box.conf_score
            curr_x1, curr_y1, curr_x2, curr_y2 = box.box

            # Print data to CLI
            print(f"Detected: {model.names[curr_cls]} | Confidence: {curr_conf:.2f} | BBox: ({curr_x1}, {curr_y1}), ({curr_x2}, {curr_y2})")
            
            # Draw Rectangles
            color = colors[curr_cls % len(colors)] 
            cv2.rectangle(frame, (curr_x1, curr_y1), (curr_x2, curr_y2), color, 2)
            cv2.putText(frame, f"{model.names[curr_cls]} {curr_conf:.2f}", (curr_x1, curr_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
